...
class Ffvae_laftr(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Laftr Discriminator
        self.adv_neurons = [args.zdim] + args.adepth * \
                    [args.awidths] + [args.num_groups-1]
        self.discriminator_laftr = MLP_cfair(self.adv_neurons)
    ...
    def vae_params(self):
        ...
        if self.data == "clr-mnist":
            ...
            return list(self.encoder.parameters()) + list(self.decoder.parameters()) + list(self.discriminator_laftr.parameters())
    ...
    def get_weighted_aud_loss(self, L, X, Y, A, A_wts, Y_wts, AY_wts):
        ...
        elif self.arch == "laftr-eqodd" or self.arch == "laftr-eqopp0" or self.arch == "laftr-eqopp1":
            A0_Y0_wt = AY_wts[0][0]
            A0_Y1_wt = AY_wts[0][1]
            A1_Y0_wt = AY_wts[1][0]
            A1_Y1_wt = AY_wts[1][1]
    ...
    def forward(self, inputs, labels, attrs, mode="train",  A_wts=None, Y_wts=None, AY_wts=None, reweight_target_tensor=None,
        ...
        A_logits = torch.squeeze(self.discriminator_laftr(zb))
        aud_loss   = - self.adv_coeff  * self.wass_loss(attrs, A_logits)
        weighted_aud_loss = self.get_weighted_aud_loss(aud_loss, inputs , labels, attrs, A_wts, Y_wts, AY_wts)
        ffvae_loss += torch.mean(weighted_aud_loss)

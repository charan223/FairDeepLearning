...
class FilterSensitive(Metric):
    ...
    def calc(
        ...
        predicted_sens = [
            pred
            for pred, sens in zip(predicted, sensitive)
            if sens == self.sensitive_filter
        ]
        sensitive_sens = [sens for sens in sensitive if sens == self.sensitive_filter]
        ...
        for sens_val in dict_of_sensitive_lists:
            ...
            filtered = [
                s
                for s, sens in zip(other_sensitive, sensitive)
                if sens == self.sensitive_filter
            ]
